...
namespace car
{
	...
	bool Checker::try_satisfy (const int frame_level)
	{
		...
		while (s != NULL)
		{
			...
			if(restartFlag){
				restart();
				cout<<"take restart, current depthBoundary = "<<depthBoundary<<std::endl;
				restartFlag = false;
				++restartTimes;
				depthBoundary++;
			}
	...
	*/
	...
	void Checker::restart(){
		...
		//about depth
		int i = 0;
		for(auto it = frameDepth.begin(); it != frameDepth.end(); ++it) cout<<"frameDeapth["<<i++<<"] = "<< *it<<std::endl;
		frameDepth.clear();
		averageDepth = 0.0;
		//
	}

	void Checker::updateFrameDepth(int frameLevel){
		int size = frameDepth.size();
		if(size >= listLength){
			if(abs(frameLevel - averageDepth) < depthBoundary){
				restartFlag = true;
			}
			averageDepth += (frameLevel - frameDepth.front())/listLength;
			frameDepth.pop_front();
			frameDepth.push_back(frameLevel); 
		}
		else{
			averageDepth = (averageDepth * size + frameLevel) / (size+1);
			frameDepth.push_back(frameLevel); 
		}
	}
}
